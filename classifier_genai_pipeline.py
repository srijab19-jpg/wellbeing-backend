...
def get_genai_guidance(classification_result: dict) -> dict:
    ...
    try:
        ...
        # gemini-3.5-flash-lite: as of Aug 21, 2026, Google's API itself
        # rejected gemini-2.5-flash-lite with "no longer available to new
        # users, use models/gemini-3.5-flash-lite instead" (confirmed via
        # a live 404 error from the Gemini API, not assumed). If this
        # breaks again later, check the error message first — Google has
        # been retiring Flash-Lite versions faster than their docs update.
